...
/***/ function(module, exports, __webpack_require__) {
	...
	(0, _jquery2.default)(function () {
		function Subtype() {
			this.$mediaImg = (0, _jquery2.default)('.media-img');
			this.$mediaVideo = (0, _jquery2.default)('.media-video');
			this.$videoCover = (0, _jquery2.default)('.video-cover');
			this.$videoObj = (0, _jquery2.default)('#video');
			this.$avatar = (0, _jquery2.default)('.avatar-author');
			this.$authorName = (0, _jquery2.default)('.author-name');
			this.$authorTime = (0, _jquery2.default)('.author-time');
			this.$text = (0, _jquery2.default)('.text');
			this.$likeCount = (0, _jquery2.default)('.like-count').find('span');
			this.$commentBar = (0, _jquery2.default)('.com-bar');
			this.$commentArea = (0, _jquery2.default)('.com-area');
			this.$QRcode = (0, _jquery2.default)('.QRcode').find('img');
			this.$bar = (0, _jquery2.default)('.bar');
			this.$download = (0, _jquery2.default)('.download');

			this.screenWidth = document.body.clientWidth;
			this.host = 'http://stage.pub.hzvb.kankanapp.com.cn/';
		}
		...
		Subtype.prototype = {
			init: function init() {
				...
				var data = {
					id: 'JgA9kLNngVz1m4PHmL9fGn///qpaiQ0fXNBvsL2p0AA=',
					// id: 'upXaFHR+jCf3YjW5YloCv3///qrV5JR/SL891ALYlAA\u003dimageDetailSplit636489193177914_54137685',
					lang: this.params.lang
				};
				// console.log(data);
				this.server(data, 'api/v3/share/postShare', function (res) {
					console.log(JSON.stringify(res));
					if (res.responseJSON.meta.statusCode == 200) {
						res = res.responseJSON.content;
						_this.showMeida(res);
						_this.showInfo(res);
					}
				}, 'get');
			...
			showMeida: function showMeida(data) {
				if (data.postType == 'image') {
					var imgHeight = data.image.height / data.image.width * this.screenWidth;
					// console.log(imgHeight);
					this.$mediaImg.css('height', imgHeight).attr('src', data.image.url).show();
				} else if (data.postType == 'video') {
					this.$videoCover.find('.cover').attr('src', data.video.coverUrl);
					this.$videoObj.attr('src', data.video.videoUrl);
					this.$mediaVideo.show();
				}
			},
			showInfo: function showInfo(data) {
				document.title = data.multiLang.appName;
				this.$avatar.css('backgroundImage', 'url(' + data.profile.avatar + ')');
				this.$authorName.text(data.profile.authorName);
				this.$authorTime.text(data.pubDate);
				this.$likeCount.text(data.likeCount);
				// this.$text.text(data.content);
				data.content && this.$text.text(data.content).show();

				data.comments = data.comments.slice(0, 5);

				var comArr = data.comments.map(function (item, index) {
					return '<div class="list">' + '<div class="avatar bg-img" style="background-image:url(' + item.avatar + ')"></div>' + '<div class="com-info">' + '<p class="com-name">' + item.nickname + '</p>' + '<p class="com-time">' + item.pubDate + '</p>' + '<p class="com-con">' + item.comment + '</p>' + '</div>' + '</div>';
				});

				if (comArr.length) {
					this.$commentBar.show();
					var result = comArr.join('');
					this.$commentArea.html(result);
					(0, _jquery2.default)('.list').last().css('border', 'none');
				}
			},
			videoPlay: function videoPlay() {
				this.$videoCover.hide();
				this.$videoObj.show()[0].play();
			},
			multiLang: function multiLang() {

				switch (this.params.lang) {
					case 'zh_CN':
						this.$bar.attr('src', './src/images/bar-zh.png');
						this.$download.attr('src', './src/images/download-zh.png');
						this.$QRcode.attr('src', './src/images/QRcode-zh.png');
						break;

					case 'zh_TW':
						this.$bar.attr('src', './src/images/bar-tw.png');
						this.$download.attr('src', './src/images/download-tw.png');
						this.$QRcode.attr('src', './src/images/QRcode-tw.png');
						break;

					case 'en_US':
					default:
						this.$bar.attr('src', './src/images/bar-en.png');
						this.$download.attr('src', './src/images/download-en.png');
						this.$QRcode.attr('src', './src/images/QRcode-en.png');
				}
			},
			server: function server(data, url, callback, type) {
		...
		a.$videoCover.on('click', function () {
			a.videoPlay();
		});
	});
